...
const globalAuthRef = globalThis as typeof globalThis & {
  __previewAuthSecret__?: string;
};
function previewAuthSecret(): string {
  globalAuthRef.__previewAuthSecret__ ??= randomBytes(32).toString("hex");
  return globalAuthRef.__previewAuthSecret__;
}
...
const authIssuer = env("GROK_AUTH_ISSUER") ?? AUTH_ISSUER_DEFAULT;
const authClientId = env("GROK_AUTH_CLIENT_ID") ?? PREVIEW_CLIENT_ID;
const authClientSecret = env("GROK_AUTH_CLIENT_SECRET") ?? PREVIEW_CLIENT_SECRET;
...
const authOAuthPlugin = authConfigured
  ? genericOAuth({
...
export const auth = betterAuth({
  ...
  plugins: [
    ...
    ...(authOAuthPlugin ? [authOAuthPlugin] : []),
