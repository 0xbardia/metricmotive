        ...
        <>
          <section className="mt-9" aria-labelledby="history-title">
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-5">
              ...
              ].map(([label, value, tone]) => <div key={String(label)} className="border-t border-rule pt-3"><p className="font-mono text-[0.64rem] uppercase tracking-[0.1em] text-graphite">{label}</p><p className={`mt-1 font-display text-3xl tabular-nums ${tone === "brick" ? "text-brick" : tone === "sage" ? "text-sage" : tone === "ochre" ? "text-ochre" : "text-carbon"}`}>{value}</p></div>)}
            </div>
          </section>
          {guards.length === 0 ? (
            <section className="product-status paper-panel mt-12 max-w-3xl p-6" aria-labelledby="empty-cases-title">
              <Badge tone="ochre">First case</Badge>
  ...
  return (
    <li>
      <Link to="/app/guards/$id" params={{ id: guard.id }} className="group flex flex-col gap-3 py-5 sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className="font-mono text-[0.64rem] uppercase tracking-[0.1em] text-graphite">v{guard.version}</span><Badge tone={guard.status === "RESOLVED" ? "sage" : guard.status === "ARMED" ? "ochre" : "graphite"}>{statusLabel(guard.status)}</Badge>{guard.recordClass === "DUPLICATE" ? <Badge tone="graphite">Duplicate · superseded</Badge> : null}</div><p className="mt-2 truncate font-display text-xl group-hover:text-ochre">{guard.motive || "Untitled motive"}</p><p className="mt-1 truncate text-sm text-graphite">{guard.metric || "No metric yet"}</p></div>
        <div className="flex items-center gap-3 sm:shrink-0"><VerdictStamp verdict={guard.verdict ?? guard.status} /><span className="font-mono text-xs text-graphite">{nextStep(guard)}</span><ArrowRight className="size-4 text-graphite transition-transform group-hover:translate-x-1" /></div>
      </Link>
    </li>
  );
}

function statusLabel(status: Guard["status"]): string {
  return { DRAFT: "Draft", ARMED: "Motive locked", EVIDENCE_SUBMITTED: "Evidence committed", RESOLVED: "Verified" }[status];
}

function nextStep(guard: Guard): string {
  if (guard.status === "DRAFT") return "Publish & lock";
...
export function AppHomeFallback() {
  return (
    <>
      <div className="max-w-4xl"><p className="font-mono text-[0.6875rem] uppercase tracking-[0.16em] text-graphite">Verification workspace</p><div className="mt-1 flex flex-wrap items-end justify-between gap-4"><div><h1 className="font-display text-4xl tracking-tight">Cases</h1><p className="mt-2 max-w-2xl text-sm leading-relaxed text-graphite">Define what the agent should achieve, capture what happened, and let GenLayer verify the difference.</p></div><Link to="/app/guards/new"><Button>Create a Motive Guard<ArrowRight className="size-4" /></Button></Link></div><div className="mt-7"><FlowRail steps={["Define", "Lock", "Run", "Review & verify"]} current={0} /></div></div>
      <section className="product-status paper-panel mt-9 max-w-3xl p-6" aria-live="polite"><Badge tone="ochre">Start here</Badge><h2 className="mt-3 font-display text-3xl tracking-tight">Connect a wallet to open your workspace.</h2><p className="mt-2 text-sm leading-relaxed text-graphite">Your Guards and Runs are private to the wallet that created them. Public verification receipts remain readable without a wallet.</p></section>
    </>
  );
}
